...
class Datatable {
    ...
    public function __construct() {
        ...
        foreach ($this->ci->data['table']['columns'] as $column) {
            $this->columns[] = $column['name'];
        }
//        $this->columns = $this->ci->data['table']['columns'];
...
//        print_r($this->columns);
     ...
     * @return mixed
     */
    public function like($key_condition, $val = NULL, $side = 'both', $backtick_protect = TRUE) {
        $this->like[] = array($key_condition, $val, $side, $backtick_protect);
        $this->ci->db->like($key_condition, $val, $side, $backtick_protect);
        return $this;
    }
     ...
     * @return mixed
     */
    public function or_like($key_condition, $val = NULL, $side = 'both', $backtick_protect = TRUE) {
        $this->or_like[] = array($key_condition, $val, $side, $backtick_protect);
        $this->ci->db->or_like($key_condition, $val, $side, $backtick_protect);
        return $this;
    }
    ...
    private function get_filtering() {
        $filters = $this->ci->input->post('filter');
//        $sWhere = '';
...
//            $this->ci->db->group_start();
            foreach ($filters as $column => $value) {
                if ($value != '') {
                    $this->ci->db->like('IFNULL(' . $this->columns[$column] . ',"")', $value);
                }
            }
...
//                if ($filter['name'] != '') {
//                    $sWhere .= $filter['name'] . " LIKE '%" . $filter['search']['value'] . "%' AND ";
//                } else {
//                    $sWhere .= $filter['data'] . " LIKE '%" . $filter['search']['value'] . "%' AND ";
//                }
//                //$sWhere .= $this->select[$filter['data']] . " LIKE '%" . $filter['search']['value'] . "%' OR ";
//            }
//        }
//        $where = substr_replace($sWhere, '', -4);
//
//        if ($where != '') {
...
//                print_r($this->columns);
//                print_r($aaData[$row_key]);
//                echo $aaData[$row_key][array_search($modkey, $this->columns)];
//                    print_r($aaData[$row_key][($this->check_cType()) ? $modkey : array_search($modkey, $this->columns)]);
//                    $aaData[$row_key][($this->check_cType()) ? $modkey : array_search($modkey, $this->columns)] = $this->exec_replace($val, $aaData[$row_key]);
//                    $aaData[$row_key][($this->check_cType()) ? $modkey : $this->columns[$modkey]] = $this->exec_replace($val, $aaData[$row_key]);
                    $aaData[$row_key][($this->check_cType()) ? $modkey : $modkey] = $this->exec_replace($val, $aaData[$row_key]);
                }
    ...
    private function get_total_results($filtering = FALSE) {
        ...
        foreach ($this->like as $val)
            $this->ci->db->like($val[0], $val[1], $val[2], $val[3]);

        foreach ($this->or_like as $val)
            $this->ci->db->or_like($val[0], $val[1], $val[2], $val[3]);
...
//                    echo $sval;
                    if ($sval == 'date_added' || $sval == 'date_modified' || $sval == 'date_deleted') {
                        $replace_string = get_date($row_data[($this->check_cType()) ? $sval : array_search($sval, $this->columns)]);
                    } else {
