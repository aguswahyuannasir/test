...
class CI_Aauth {
    ...
    public function control($perm_par = FALSE) {
        ...
        $this->update_activity();
        if (!$this->is_loggedin()) {
            if (!in_array($this->CI->router->fetch_class(), ['auth']) && !in_array($this->CI->uri->segment(1), ['display'])) {
        ...
        if ($perm_par == FALSE) {
        ...
        } elseif (!$this->is_allowed($perm_id) OR ! $this->is_group_allowed($perm_id)) {
            if ($this->config_vars['no_permission']) {
                ...
                show_error($this->CI->lang->line('aauth_error_no_access'));
                die();
            }
        }
    }

    public function controlv2($perm_par = FALSE, $module_level = 3){
        $this->CI->load->helper('url');

        $user_session = $this->CI->session->user;
        $this->update_activity();
        if ($user_session->role != "owner") {
            $check_module    = $this->is_assign_module($perm_par, $user_session->id, $module_level);
            if ($check_module == FALSE) {
                $check_submodule = $this->is_assign_module_sub($perm_par, $user_session->id, $module_level);
                if ($check_submodule == FALSE) {
                    $this->config_vars['no_permission'] = TRUE;
                }
            }

            if ($this->config_vars['no_permission'] == TRUE) {
                redirect('errors?code=401&response=Sorry you do not have permission to access this page.');
            }
            return TRUE;
        }else{
            return TRUE;
    ...
    public function is_assign_module($alias, $user_id, $module_level){
        $this->aauth_db->join("sys_module AS sm", "sm.id = md.module_id");
        $this->aauth_db->where("member_id", $user_id);
        $this->aauth_db->where("sm.alias", $alias);
        $this->aauth_db->where("sm.module_level", $module_level);
        $query  = $this->aauth_db->get($this->config_vars['module']." AS md");
    ...
    public function is_assign_module_sub($alias, $user_id, $module_level){
        $this->aauth_db->join("sys_module_sub AS sms", "sms.id = ms.module_sub_id");
        $this->aauth_db->join("sys_module AS sm", "sm.id = ms.module_id");
        $this->aauth_db->where("ms.member_id", $user_id);
        $this->aauth_db->where("sms.alias", $alias);
        $this->aauth_db->where("sm.module_level", $module_level);

        $query  = $this->aauth_db->get($this->config_vars['module_sub']." AS ms");
        $result = $query->num_rows();
        if ($result > 0) {
            return TRUE;
        }else{
    ...
    public function create_user($email, $pass, $username = FALSE, $group = FALSE) {
     ...
     * @return bool Update fails/succeeds
     */
    public function update_user($user_id, $email = FALSE, $pass = FALSE, $username = FALSE, $group = FALSE) {
...
//        if (!$valid || empty($data)) {
        ...
        if ($group) {
            $this->aauth_db->where('user_id', $user_id)
                    ->delete($this->config_vars['user_to_group']);
            $this->add_member($user_id, $group);
        }
        return true;
    }
    ...
    public function delete_perm($perm_par) {
        ...
        if ($query->num_rows() == 0) {
            $this->error($this->CI->lang->line('aauth_error_no_perm'));
            return FALSE;
        }
    ...
    public function is_allowed($perm_par, $user_id = FALSE) {
        ...
        // JUST RETURN TRUE, BECAUSE TABLE perm_to_user not exist
        return TRUE;
        $perm_id = $this->get_perm_id($perm_par);
    ...
    public function in_group($check_group, $id = false, $check_all = false) {
        $id || $id = $this->CI->session->userdata('id');
        ...
        if (isset($this->_cache_user_in_group[$id])) {
            ...
            $users_groups = $this->aauth_db->select('ag.id, ag.name, ag.definition')
                            ->join('aauth_groups ag', 'aug.group_id = ag.id', 'left')
                            ->where('aug.user_id', $id)
                            ->get('aauth_user_to_group aug')->result();
            $groups_array = array();
