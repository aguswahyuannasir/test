    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; }
            ...
            #terms h5 { text-transform: uppercase; font: 13px dejavusanscondensed; letter-spacing: 10px; border-bottom: 1px solid black; padding: 0 0 8px 0; margin: 0 0 8px 0; }
            #terms textarea { width: 100%; text-align: center;}

            #signature { text-align: center; margin-top: 20px; width: 100%; }
            #signature td{ border: none; word-wrap: break-word; }
            #signature img{ height: 100px; width: 100px; }
            #signature hr{ width: 95%; margin: 0 auto; }
        </style>
        ...
        <div id="page-wrap">
                ...
                <table id="meta">
                    <tr>
                        <td rowspan="5" style="border: 1px solid white; border-right: 1px solid black; text-align: left" width="62%">
                            ...
                            <?php
                            echo ($data->vendor_company) ? $data->vendor_company . '<br>' : '';
                            echo $data->vendor_name . '<br>';
                    ...
                    <?php if (isset($template->term)) { ?>
                        <tr>
                            ...
                            <td><?php echo ($data->term) ? $data->term_name : '-'; ?></td>
                        </tr>
                    <?php } ?>
                    <tr>
            ...
            <table id="items">
                <tr>
                    ...
                    <?php if (isset($template->product_tax)) { ?>
                        <th align="center"><?php echo lang('tax'); ?></th>
                    <?php } ?>
                    <th align="right"><?php echo lang('total'); ?></th>
                </tr>
                <?php
                if (isset($template->product_tax)) {
                    $col_span_total = 3;
                } else {
                    $col_span_total = 2;
                }
                ...
                foreach ($products->result() as $product) {
                    if ($product->tax) {
                        if (!isset($taxes[$product->tax])) {
                            $taxes[$product->tax] = ['name' => $product->tax_name, 'amount' => $product->tax_amount, 'rate' => $product->tax_rate];
                        } else {
                            $taxes[$product->tax]['amount'] += $product->tax_amount;
                        }
                    }
                    ?>
                    <tr>
                        ...
                        <?php if (isset($template->product_tax)) { ?>
                            <td align="center"><?php echo $product->tax_total; ?></td>
                        <?php } ?>
                ...
                <tr>
                    <td class="blank"> </td>
                    <td colspan="<?php echo $col_span_total; ?>" class="total-line"><?php echo lang('subtotal'); ?></td>
                    <td class="total-value"><div id="subtotal"><?php echo money_formating($data->subtotal, $data->currency_prefix, $data->currency_decimal_digit, $data->currency_decimal_separator, $data->currency_thousand_separator, $data->currency_suffix); ?></div></td>
                </tr>
                <?php
                if ($taxes) {
                    foreach ($taxes as $tax) {
                        echo '<tr><td class="blank"> </td><td colspan="' . $col_span_total . '" class="total-line">' . $tax['name'] . ' ' . $tax['rate'] . '%' . '</td><td class="total-value"><div id="subtotal">' . money_formating($tax['amount'], $data->currency_prefix, $data->currency_decimal_digit, $data->currency_decimal_separator, $data->currency_thousand_separator, $data->currency_suffix) . '</td></div></tr>';
                    }
                }
                ?>
                <tr>
                    <td class="blank"> </td>
                    <td colspan="<?php echo $col_span_total; ?>" class="total-line balance"><?php echo lang('total'); ?></td>
                    <td class="total-value balance"><div class="due"><?php echo money_formating($data->total, $data->currency_prefix, $data->currency_decimal_digit, $data->currency_decimal_separator, $data->currency_thousand_separator, $data->currency_suffix); ?></div></td>
                </tr>
            </table>
